...
def test_prefill_plp_round_trips_cached_prompt_logprobs():
    ...
    # All blocks should be available again after both requests are freed.
    assert manager.block_pool.free_block_queue.num_free_blocks == 10
    # Only req0 allocated blocks in this round-trip test. req1 only reads
    # the cached prefix metadata, so the free queue returns unallocated blocks
    # first, then req0's tail block, then its shared prefix blocks.
    assert [
        b.block_id for b in manager.block_pool.free_block_queue.get_all_free_blocks()
