...
def test_model_runner_expert_indices_are_returned_when_request_finishes():
    scheduler = object.__new__(Scheduler)
    ...
    scheduler.routed_experts_reader = None
    scheduler.recompute_kv_load_failures = False
    scheduler.make_stats = Mock(return_value=None)
    scheduler.max_model_len = 128
    ...
    first_model_output = ModelRunnerOutput(
        ...
        prompt_logprobs_dict={},
        pooler_output=[],
        expert_indices=first_expert_indices,
    )

    assert scheduler.update_from_output(first_output, first_model_output) == {}

    second_output = SchedulerOutput(
        scheduled_new_reqs=[],
        scheduled_cached_reqs=CachedRequestData.make_empty(),
        num_scheduled_tokens={request.request_id: 1},
        total_num_scheduled_tokens=1,
    ...
    second_expert_indices = np.array(
        [
            [[14, 15]],
            [[24, 25]],
        ],
    ...
    second_model_output = ModelRunnerOutput(
        req_ids=[request.request_id],
        req_id_to_index={request.request_id: 0},
        sampled_token_ids=[[123]],
        logprobs=None,
        prompt_logprobs_dict={},
        pooler_output=[],
        expert_indices=second_expert_indices,
    )

    engine_core_outputs = scheduler.update_from_output(
        second_output, second_model_output
    )
