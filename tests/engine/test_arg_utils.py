...
def test_device_ids_compose_with_visible_device_namespace(
    monkeypatch: pytest.MonkeyPatch,
):
    monkeypatch.setenv(current_platform.device_control_env_var, "4,5")

    engine_args = EngineArgs(model="m", device_ids=[0, 1])

    assert engine_args._resolve_device_ids() == [4, 5]


def test_cpu_control_mapping_preserves_empty_visible_devices(
    monkeypatch: pytest.MonkeyPatch,
):
    device_control_env_var = current_platform.device_control_env_var
    monkeypatch.setenv(device_control_env_var, "")
    ...
    set_assigned_physical_gpu_ids_for_dp_rank(
        config,
        local_dp_rank=0,
        user_assigned_gpu_ids=[4, 5],
    )

    assert config.parallel_config.assigned_physical_gpu_ids == [4, 5]
    assert os.environ[device_control_env_var] == ""
