...
"""Detect the manylinux platform tag and retag a wheel in place.
...
``winfo.sym_policy.name``. We then use PyPA's ``wheel tags`` command so the
filename, internal WHEEL metadata, and RECORD hashes stay consistent.
...
def retag_wheel(wheel_path: Path, new_platform_tag: str) -> Path:
    """Replace the platform tag in both the filename and wheel metadata."""
    ...
    subprocess.run(
        [
            ...
            "tags",
            "--remove",
            f"--platform-tag={new_platform_tag}",
            str(wheel_path),
        ],
        check=True,
        capture_output=True,
    ...
    if not new_path.is_file():
        raise RuntimeError(f"wheel tags did not produce {new_path}")
    return new_path
...
def main() -> int:
    parser = argparse.ArgumentParser(
        ...
        "auditwheel and rewrite its compatibility tags."
    )
    parser.add_argument(
        "wheel",
        type=Path,
        help="Path to the wheel to inspect and retag.",
    )
    ...
    # Keep expected analysis, retagging, and filesystem failures to one CI line.
    try:
        new_tag = detect_platform_tag(wheel_path)
        print(f"detected platform tag: {new_tag}", file=sys.stderr)
        new_path = retag_wheel(wheel_path, new_tag)
    except (
        AuditwheelError,
        ValueError,
        OSError,
        ...
        print(
            f"error: failed to retag {wheel_path.name}: "
